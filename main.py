...
def main():
    ...
    parser.add_argument('--template-path', default=None, type=str, help='球场模板图像路径（可选）；不提供时自动使用 CourtKeyNet 从视频检测球场')
    parser.add_argument('--courtkeynet-root', default=None, type=str, help='CourtKeyNet 项目目录（含 models/），默认读取 COURTKEYNET_ROOT 环境变量或内置默认路径')
    parser.add_argument('--courtkeynet-weights', default=None, type=str, help='CourtKeyNet 权重文件路径，默认读取 COURTKEYNET_WEIGHTS 环境变量或内置默认路径')
    ...
    parser.add_argument('--shuttle-model', default='yolo', choices=['yolo', 'tracknet', 'tracknet_v4', 'ensemble'], help='羽毛球检测方式：yolo、tracknet、tracknet_v4 或 ensemble（三模型融合）')
    parser.add_argument('--tracknet-v4-weights', default='weights/tracknet_v4/tracknet_v4_type_b.keras', help='TrackNetV4 Type-B Keras 权重路径')
    ...
    if args.courtkeynet_root:
        os.environ["COURTKEYNET_ROOT"] = args.courtkeynet_root
    if args.courtkeynet_weights:
        os.environ["COURTKEYNET_WEIGHTS"] = args.courtkeynet_weights
    ...
    system = BadmintonAnalysisSystem(
        ...
        tracknet_v4_weights_path=args.tracknet_v4_weights,
    )
