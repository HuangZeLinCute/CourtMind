...
class ShuttlecockTracker:
    ...
    def detect_ball(self, frame, conf=0.18, roi_corners=None):
        candidates = self.detect_candidates(frame, conf, roi_corners)
    ...
    def detect_candidates(self, frame, conf=0.18, roi_corners=None):
        """Return all geometrically valid YOLO boxes before temporal selection."""
        t0 = time.time()
        try:
        ...
        return self._extract_candidates(ball_results, frame.shape, roi_corners)

    def set_detection(self, x, y, visible=True, confidence=None):
        """Feed an externally produced shuttlecock position (e.g. from
        TrackNetV3) into the tracker without running per-frame YOLO inference.

        Returns the same ``[x, y]`` / ``[0, 0]`` contract as ``detect_ball``.
        """
        if not visible:
            self.last_candidate = None
            self.last_detection = {
        ...
        self.last_candidate = {"point": point, "confidence": confidence, "source": "external"}
        self.last_detection = {
            "visible": True,
            "accepted": False,
            "image": list(point),
    ...
    def update_trajectory(self, ball_position, roi_corners=None, temporal_validated=False):
        ...
        if not temporal_validated and self._is_outlier(point):
            self._record_missing_detection()
            self._mark_detection_rejected()
            return [0, 0]
    ...
    def point_in_roi(self, point, roi_corners=None):
        """Public ROI check used by multi-model fusion before voting."""
        return self._point_in_roi(point, roi_corners)
